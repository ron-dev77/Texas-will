...
import { articleArabicFromRoman } from '@/lib/will-article-numbering'
...
function buildSntArticleFromRow(row: SntTrustRow, articleRoman: string): SpecialNeedsArticle {
  const n = articleArabicFromRoman(articleRoman)
...
export function buildSpecialNeedsArticles(
  answers: Answers,
...
export function buildSpecialNeedsTrustArticle(
  answers: Answers,
  options: SntClauseOptions = {},
): SpecialNeedsArticle | null {
  const articles = buildSpecialNeedsArticles(answers, options)
